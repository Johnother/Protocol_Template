...
\\
Zusätzlich
steht eine Infrarot-Sichtkarte (IR-Sichtkarte) sowie eine Kamera, die an einen Monitor angeschlossen ist, zur Verfügung, um den Strahlengang des Lasers verfolgen zu können und die Fluoreszenz des Rubidiums zu beobachten.
...
\begin{figure}
    ...
    \caption{Diodenlaser mit oberem Knopf (oben links, eingesteckter Inbusschlüssel) und unterem Knopf (unten rechts).}
    \label{fig:diodenlaser}
\end{figure}
\\
\subsubsection{Anpassung der Wellenlänge anhand von Rubidiumfluoreszenz}
\label{sec:durch2}
...
das Spektrometer eingebracht. Die Kamera sollte an das dafür vorgesehenne Sichtfenster in der Rubidiumzelle gestellt werden, um das Fluoreszenzlicht auf dem Monitor beobachten zu können.
...
zunächst der Seitliche Knopf in beide Richtungen jeweils um Maximal eine ganze Umdrehung gehdreht und dann wieder in die Ausgangspostion gebracht. Ist kein Fluoreszenzlicht zu sehen wird nun die 
Spannung um etwas $\qty{0.33}{\volt}$ erhöht und das Drehen des Knopfes wiederholt. Dabei lässt sich die Spannung maximal auf $\qty{8.43}{\volt}$ erhöhen. Ist weiterhin kein Fluoreszenzlicht zu sehen
wird der Prozess für eine andere Temperatur wiederholt. Diese wird dabei jeweils in $\qty{2}{\degreeCelsius}$ Schritten angepasst. Ist immer noch keine Fluoreszenz zu erkennen, wird das Spektrometer 
...
wird wieder der Monitor beobachtet und der Laser über die beiden Knöpfe weiter feinjustiert, bis auf dem Bildschirm das Fluoreszenzlicht zu sehen ist. 

...
\label{Durchführung}
Zur Durchführung des Versuchs leigt ein LRC-Messgerät, ein Signalgenerator, ein NIM-Pulser und ein Oszilloskop vor.
...
\begin{equation}
    ...
    \label{eqn:dieltheo}
\end{equation}
Es werden sechs verschiedene Kabel untersucht, deren Längen in \autoref{tablentheo} zu finden sind.
...
Dazu wird das Kabel direkt an das LRC-Messgerät angeschlossen und für Frequenzen von $\qty{1}{\kilo\hertz}$ bis $\qty{100}{\kilo\hertz}$ ausgenommen.
Das gerät kann aber keine Frequenzen zwischen $\qty{20}{\kilo\hertz}$ und $\qty{100}{\kilo\hertz}$ vermessen, weshalb hier ein Sprung in der Messreihe gemacht werden muss. 
Für die Messung von $L$ und $R$ muss an das Ende des Kabels ein Kurzschluss angeschlossen werden und für die Messung von $C$ bleibt das Ende offen. 
\\\\
...
Dabei wird er NIM-Pulser verwendet und die sechs verschiedenen Kabel nacheinander mittels T-Stück und weiterem kurzen Kabel an Oszilloskop und Pulser angeschlossen.
...
Dazu wird ebenfalls der Aufbau in \autoref{fig:abschlüsse} mit dem Kabel 3 verwendet. 
Als Pulser wird der Signalgenerator genutzt.
An diesem wird eine Recheckspannung eingestellt.
Es werden dann nacheinander die Abschlüsse 1, 2, 3, und 8 des Kastens sowie ein offener Abschluss, ein Kurzschluss, ein $\qty{50}{\ohm}$ und ein $\qty{75}{\phm}$ Widerstand als Abschluss angeschlossen.
Für jeden Abschluss wird das Signalbild am Oszilloskop abgespeichert. 
Dieses kann dan mit den theoretischen Verläufen abgeglichen werden und es kann die Signalspannung abgelesen werden.
\\\\
Zur Meesung der Reflexionsfaktoren bei Reihenschaltung eines $\qty{50}{\ohm}$ und eines $\qty{75}{\ohm}$ Kabels wird ebenfalls der Aufbau in \autoref{fig:aufbau} benutzt. 
Hier wird Kabal 3 direkt am Oszilloskop angeschlossen und das Ende von Kabel 3 mit Kabel 5 durch entsprechenden Adapter verbunden. 
...
Daraus lassen sich die Reflexionsfaktoren bestimmten.
\begin{figure}
    \centering

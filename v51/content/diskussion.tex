...
Die theoretischen und experimentellen Werte der Leerlaufverstärkung sowie ihre relative Abweichung nach \autoref{eqn:relabw} sind für die vier Messreihen in \autoref{tab:leer} aufgelistet.
\begin{table}
  \centering
  ...
  \begin{tblr}{
      colspec = {S[table-format=1.0] S[table-format=2.2] S[table-format=1.2] S[table-format=1.3] S[table-format=1.3] S[table-format=1.1] },
      row{1} = {guard, mode=math},
      vline{3,5} = {2}{-}{text=\clap{$\pm$}},
    }
    \toprule
    \mathrm{Messreihe} & \SetCell[c=2]{c} V_{\mathrm{theo}} & & \SetCell[c=2]{c} V_{\mathrm{exp}} & & \symup{\Delta}V \mathbin{/} \% \\
    \midrule
    1   & 10   & 0.14 & 9.44 & 0.02   & 5.6  \\
    2   & 6.67 & 0.09 & 6.36 & 0.02   & 4.6  \\
    3   & 2.13 & 0.03 & 1.966 & 0.002 & 7.7  \\
    4   & 1.47 & 0.02 & 1.39 & 0.04   & 5.4  \\
    \bottomrule
  \end{tblr}
\end{table}
...
\begin{table}
  \centering
  \caption{Experimentell bestimmte Werte für das Bandbreitenprodukt.}
  \label{tab:bandwidth}
  \sisetup{table-format=1.1, per-mode=reciprocal}
  \begin{tblr}{
      colspec = {S[table-format=1.0] S[table-format=3.0] S[table-format=2.0] },
      row{1} = {guard, mode=math},
      vline{3} = {2}{-}{text=\clap{$\pm$}},
    }
    \toprule
    \mathrm{Messreihe} & \SetCell[c=2]{c} V\cdot B \mathbin{/} \unit{\kilo\hertz} & \\
    \midrule
    1 & 205 & 4  \\
    2 & 238 & 13 \\
    3 & 240 & 7  \\
...
Für den Umkehr-Integrator sollte nach \autoref{equ:fint} die Steigung der Gerade $m = -1$ sein, für den Differenzierer gemäß \autoref{equ:fdiff} $m=1$.
Die Fit-Funktionen liefern für die Steigungen
\begin{align*}
    b_{\mathrm{int,exp}} &= \qty{-0.8872(0.0002)}{}, & b_{\mathrm{diff,exp}} &= \qty{0.8925(0.0013)}{}. \\
\end{align*}
Die Abweichungen zu den Theoriewerten betragen
\begin{align*}
    \symup{\Delta}b_{\mathrm{int}} &= \qty{11.28(0.02)}{\%}, & \symup{\Delta}b_{\mathrm{diff}} &= \qty{10.70(0.13)}{\%}. \\
\end{align*}
...
\begin{align*}
    U_{\mathrm{Schwell,sin}} &= \pm\qty{1.402(0.0005)}{\volt}, & \overline{U}_{\mathrm{Schwell,tri}} &= \pm(1.40945\pm0.009)\,\unit{\volt}. \\
\end{align*}
Die Abweichung der beiden Werte zueinander ist erkennbar gering. Zum theoretischen Wert $U_{\mathrm{Schwell,theo}} = \pm\qty{1.36(0.02)}{\volt}$ betragen die relativen Abweichungen nach \autoref{eqn:relabw}
\begin{align*}
    \symup{\Delta}U_{\mathrm{Schwell,sin}} &= \qty{3.1(1.5)}{\%}, & \symup{\Delta}{U}_{\mathrm{Schwell,tri}} &= \qty{3.6(1.6)}{\%}. \\
\end{align*}

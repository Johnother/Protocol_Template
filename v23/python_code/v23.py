...
ax.plot(amp_9[:,0],amp_9[:,1],label=r"Aufspaltung 6mm Ring")
secax = ax.secondary_xaxis("top")
...
ax.plot(amp_12[:,0],amp_12[:,1],label=r"Aufspaltung 9mm Ring")
...
d = np.linspace(0,50)
fig, ax = plt.subplots(layout="constrained")
ax.errorbar([3,6,9],F_N,yerr=F_S,fmt="x",capsize=2,label=r"Messwerte $\increment f$")
popt, pcov = curve_fit(lin,[3,6,9],F_N,sigma=F_S,absolute_sigma=True)
